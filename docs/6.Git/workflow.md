...
1. Install Git if it is not already installed [Setup](https://hpc2n.github.io/bioinformatics-hpc/6.Git/setup/)
2. Configure Git [Configure](https://hpc2n.github.io/bioinformatics-hpc/6.Git/setup/#configure__git__all__os) 
3. Test Git [Test](https://hpc2n.github.io/bioinformatics-hpc/6.Git/setup/#test__your__git__installation) 
...
2. Create SSH keys at upload/add them [SSH keys](https://hpc2n.github.io/bioinformatics-hpc/6.Git/setup/#create__a__new__ssh__key__for__github) 
...
git init
```
3. Add/create once or more files. One of them should preferrably be ``README.md`` with info about the repository. 
4. Stage and commit the file(s):
```bash
git add README.md
git commit -m "A great commit message"
```
If you want to add all modified and new files in the current directory and below it, you can use (be careful): 
```bash
git add .
```
...
5. Create a repository on GitHub 
    ...
    - Name it the same as your local project folder. (Here, ``myproject``). 
    - Do not choose to initialize it with a readme - you are using the file in your local project! 
...
3. 
    - Check with ``git status`` if you have something that is not added/committed.
    - If so, ``git add FILE(S)``, ``git commit -m "Write a good commit message so you can later recognize what is in this change"``
    - Do ``git pull --rebase`` in case others have updated something while you worked. 
    - Then do ``git push``
