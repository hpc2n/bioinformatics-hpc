# Exercises: FAIR in Practice (Lecture 15)

Exercises accompany [Lecture 15](../../15.fair-in-practice/fair-in-practice.md).

| File | Description |
|------|-------------|
| [blast_tp53.sh](blast_tp53.sh) | Slurm job script for local blastp — one of the three ways to run BLAST covered in this session |
| [query_plantgenie.sh](query_plantgenie.sh) | PlantGenIE API query script |

For BLAST background (what it is, how it works, how to interpret results), see the [BLAST background reference](../../12.blast/blast.md).

## Exercise 2 dataset

**ERP016242** — AspWood (*Populus tremula* wood formation)
Browse: [ebi.ac.uk/ena/browser/view/ERP016242](https://www.ebi.ac.uk/ena/browser/view/ERP016242)

Full commands in the [lecture handout](../../15.fair-in-practice/fair-in-practice.md).
